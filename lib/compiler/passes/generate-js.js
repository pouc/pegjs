...
function generateJS(ast, options) {
  ...
  function generateRuleFooter(ruleNameCode, resultCode) {
    ...
    if (options.cache) {
      parts.push([
        '',
		'if(! peg$resultsCache.maxKey) { peg$resultsCache.maxKey = 100000000 };',
		'if(key > peg$resultsCache.maxKey) {',
		'	var tmp = { maxKey: peg$resultsCache.maxKey + 100000000 };',
		'	Object.keys(peg$resultsCache).forEach(fekey => { if(fekey !== "maxKey" && peg$resultsCache[fekey].hit > 10) { tmp[fekey] = peg$resultsCache[fekey]; tmp[fekey].hit = 0; }})',
		'	peg$resultsCache = tmp;',
		'}',
        'peg$resultsCache[key] = { nextPos: peg$currPos, result: ' + resultCode + ', hit: 0 };'
      ].join("\n"));
    }
